...
let sellerStruct = [
    ...
    it("LazyAuction Function", async () => {
      ...
      await myToken.connect(seller).approve(marketPlace.address, NFTPrice);
    ...
    it("Should check the calculate Royality and Platform fee", async () => {
      ...
      await myToken.transfer(add1.address, nftPrice);
      // console.log("oldBuyBal",await myToken.balanceOf(add3.address));
     ...
     await myToken.connect(add1).approve(marketPlace.address, nftPrice);
      ...
      let OldPlatFormBal = await myToken.balanceOf(marketPlace.address);
      console.log("OLd PlatFormBal", OldPlatFormBal);
        // console.log(await myToken.balanceOf(add1.address));
      ...
      let newPlatFormBal = await myToken.balanceOf(marketPlace.address);
